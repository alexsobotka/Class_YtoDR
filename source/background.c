...
/* Y particle */
  if (pba->has_yp == _TRUE_) {
    /* Pass value of rho_yp to output */
    if (pvecback_B[pba->index_bi_rho_yp] > pow(10.,-24.) ){
      pvecback[pba->index_bg_rho_yp] = pvecback_B[pba->index_bi_rho_yp];
    }
    else{ //force rho_yp to be zero once sufficiently decayed away in order to avoid numerical issues
      pvecback[pba->index_bg_rho_yp] = 0.0;
    }
    rho_tot += pvecback[pba->index_bg_rho_yp];
    p_tot += 0.;
    rho_m += pvecback[pba->index_bg_rho_yp];
  }

  /* Decay product of Y particle, yr. Can be split into some fraction of DR and photons*/
  if (pba->has_yr == _TRUE_) {
    pvecback[pba->index_bg_rho_yr] = pvecback_B[pba->index_bi_rho_yr];
  }
...
int background_w_fld(
  ...
  case EDE:
    ...
    if (pba->has_yp == _TRUE_)
      class_stop(pba->error_message,"Early Dark Energy not compatible with decaying Y particle because we omitted to code the calculation of a_eq in that case, but it would not be difficult to add it if necessary, should be a matter of 5 minutes");
...
int background_indices(
  ...
  if ((pba->N_ncdm != 0.) && (pba->has_yp==_TRUE_))
    pba->has_ncdm = _TRUE_;
  ...
  /* - index for Y particle */
  class_define_index(pba->index_bg_rho_yp,pba->has_yp,index_bg,1);

  /* - index for Y particle radiation products */
  class_define_index(pba->index_bg_rho_yr,pba->has_yr,index_bg,1);
  ...
  class_define_index(pba->index_bi_rho_dr,pba->has_dcdm,index_bi,1);

  /* -> energy density in Y particle */
  class_define_index(pba->index_bi_rho_yp,pba->has_yp,index_bi,1);

  /* -> energy density in Y decay product */
  class_define_index(pba->index_bi_rho_yr,pba->has_yr,index_bi,1);
...
int background_solve(
  ...
  short is_dr_converged = _FALSE_;
  double Omega0_dr_used = 0.0;
  ...
  /*loop over the background calculation in order to calculate Omega0_dr and adjust Omega0_lambda accordingly.*/
  /******************************************** BEGIN WHILE LOOP **********************************************/
  while(is_dr_converged == _FALSE_){
  ...
  if (pba->has_yp == _TRUE_) {
    pba->Omega0_yp = pvecback_integration[pba->index_bi_rho_yp]/pba->H0/pba->H0;
  }  
  if (pba->has_yr == _TRUE_) {
    pba->Omega0_yr = pvecback_integration[pba->index_bi_rho_yr]/pba->H0/pba->H0;
  }
  ...
  for (index_loga=0; index_loga < pba->bt_size; index_loga++) {
    ...
    //is_dr_converged = _TRUE_;
    if (pba->has_yp == _TRUE_ && pba->has_dr == _TRUE_){
      //pba->Omega0_dr = pvecback_integration[pba->index_bi_rho_dr]/pba->H0/pba->H0;
      pba->Omega0_dr = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_dr]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit];
      pba->Omega0_lambda -= pba->Omega0_dr;  //we remove the DR contribution that we had "forgotten"
      Omega0_dr_used = pba->Omega0_dr;
      if(pba->background_verbose>0)printf("adjusted Omega_Lambda to incorporate the dark radiation contribution; new Omega_Lambda = %e  \n",pba->Omega0_lambda);

      /*test that the budget equation is satisfied or loop.*/
      // if(fabs(pba->Omega0_dr-Omega0_dr_used)<pba->precision_loop_over_background){
      if(fabs(pba->Omega0_dr-Omega0_dr_used)<pow(10.0,-9)){
        /* default is 1e-3 */
        is_dr_converged = _TRUE_;
      }
      else{
        ...
        //no DR so we ignore the loop and continue on with background calculations.
        is_dr_converged = _TRUE_;
    } 
  }
/*********************************************************** END WHILE LOOP **********************************************/
...
int background_initial_conditions(
  ...
  double f,Omega_rad, rho_rad, rho_m;
  int counter,is_early_enough,n_ncdm;
  double scf_lambda;
  double rho_fld_today;
  double tilde, c_term, d_term, alpha_term, beta_term, aGamma_ai, apivot_ai, rho_yp_i, alpha_nu, expansion;
  double Hpivot, apivot, late_decay=0.0, early_decay=0.0;
  double w_fld,dw_over_da_fld,integral_fld;
  short is_ncdm_pivot_converged = _FALSE_;
  ...
  */
  ...
  rho_m = (pba->Omega0_b + pba->Omega0_cdm)*pow(pba->H0,2)/pow(a,3);
  ...
  if (pba->has_yp == _TRUE_) {
    ...
    tilde = pba->Gamma_yp/pvecback[pba->index_bg_H];
    /* Depending on when the decay happens, the massive neutrino may be relativisitc or it may be non-relativistic.*/
    /* We need to figure out when this transition happens (a_pivot), and then compare when the decay occurs to a_pivot. */
    apivot_ai = (3.15/pba->m_ncdm_in_eV[0])*(pba->T_cmb)*_k_B_eV_*pow(4./11., 1./3.)/a;
    apivot = apivot_ai*a;
    //printf("apivot=%g\n", apivot);
    /* As a first guess, calculate H(a_pivot). If H(a_pivot) > Gamma, its a safe bet that a_Gamma < a_pivot */
    Hpivot = pba->H0*pow(( Omega_rad*pow(apivot, -4.) + (pba->Omega0_b + pba->Omega0_cdm)*pow(apivot, -3.) + pba->Omega0_lambda ), 0.5);   //[1/Mpc]
    while (is_ncdm_pivot_converged == _FALSE_){

      if ((pba->Gamma_yp >= Hpivot) || (early_decay == 1.0)){

        /*Do calculation in order to find a_Gamma, assuming a_Gamma < a_pivot. Then check to see if indeed true*/
        d_term = -(pba->R_Gamma_yp + 1)/pow(tilde,2.);
        c_term = (rho_m/rho_rad)*d_term;
        alpha_term = ( 4 * pow(2./3, 1/3.) * d_term   ) / pow( 9*pow(c_term,2.) + pow(3, 0.5) * pow(27.*pow(c_term,4) - 256*pow(d_term,3.), 0.5)  , 1./3.);
        beta_term = (4./3.)*(d_term/alpha_term);
        // printf("tilde=%g\n", 10, tilde);
        // printf("d=%.*f\n", 10, d_term);
        // printf("c=%.*f\n", 10, c_term);
        // printf("alpha=%.*f\n", 10, alpha_term);
        // printf("beta=%.*f\n", 10, beta_term);
        // printf("alpha+beta=%.*f\n", 10, alpha_term + beta_term);
        aGamma_ai = 0.5*pow(alpha_term + beta_term , 0.5) + 0.5*pow( -alpha_term - beta_term - (2*c_term)/(pow(alpha_term + beta_term, 0.5))  , 0.5);

        /* alpha and beta are extremely close in magnitude so alpha + beta is an extremely small number close to zero. This causes numerical issue for larger tilde */
        /* Instead, take the first few order terms of alpha + beta expansion */
        //expansion = 
        //aGamma_ai = 0.5*pow(expansion , 0.5) + 0.5*pow( -expansion - (2*c_term)/(pow(expansion, 0.5))  , 0.5);

        /* if a_Gamma < a_pivot is indeed true, set intial condition for rho_y and end while loop */
        if (aGamma_ai/apivot_ai <= 1.0){
          //printf("early aGamma/apivot=%g\n", aGamma_ai/apivot_ai);
          rho_yp_i = pba->R_Gamma_yp*((rho_rad/aGamma_ai) + rho_m);
          pvecback_integration[pba->index_bi_rho_yp] = rho_yp_i;
          pba->aGamma_ai = aGamma_ai;
          is_ncdm_pivot_converged = _TRUE_;
        }
        /* otherwise, if a_Gamma < a_pivot turns out not to be true, switch to late decay*/
        else{
          late_decay = 1.0;
        }
      }

      if ((pba->Gamma_yp < Hpivot) || (late_decay == 1.0)){
      /*Do calculation in order to find a_Gamma, assuming a_Gamma > a_pivot. Then check to see if indeed true*/
        alpha_nu = (1.0 + (7./8.)*(pba->N_ur)*pow(4./11., 4./3.))/(1.0 + (7./8.)*(3.044)*pow(4./11., 4./3.));
        d_term = -(pba->R_Gamma_yp + 1)/pow(tilde,2.);
        c_term = ((rho_m/rho_rad) + (1-alpha_nu)/(apivot_ai))*d_term;
        alpha_term = ( 4 * pow(2./3, 1/3.) * d_term   ) / pow( 9*pow(c_term,2.) + pow(3, 0.5) * pow(27.*pow(c_term,4) - 256*pow(d_term,3.), 0.5)  , 1./3.);
        beta_term = (4./3.)*(d_term/alpha_term);
        ...
        /* alpha and beta are extremely close in magnitude so alpha + beta is an extremely small number close to zero. This causes numerical issue for larger tilde */
        /* Instead, take the first few order terms of alpha + beta expansion */
        //expansion = -(8.195638656616211*pow(10,-8.)*pow(tilde*pow(10.,15.), -0.5)) + (2.625*pow(10,-6)*pow(tilde*pow(10.,15.), -1 )) - (6.620384183831123*pow(10,-35)*pow(tilde*pow(10.,15.), -1.5) ) - (4.306640624999998*pow(10,-33)*pow(tilde*pow(10.,15.), -2.) );
        //aGamma_ai = 0.5*pow(expansion , 0.5) + 0.5*pow( -expansion - (2*c_term)/(pow(expansion, 0.5))  , 0.5);

        /* if a_Gamma > a_pivot is indeed true, set intial condition for rho_y  */
        if (aGamma_ai/apivot_ai > 1.0){
          //printf("late aGamma/apivot=%g\n", aGamma_ai/apivot_ai);
          rho_yp_i = pba->R_Gamma_yp*(rho_rad*(alpha_nu/aGamma_ai) + rho_m + rho_rad*(1-alpha_nu)/(apivot_ai)   );
          pvecback_integration[pba->index_bi_rho_yp] = rho_yp_i;
          pba->aGamma_ai = aGamma_ai;
          is_ncdm_pivot_converged = _TRUE_;
        }
        /* otherwise, if a_Gamma > a_pivot turns out not to be true, switch back to early decay*/
        else{
          early_decay = 1.0;
        }
      }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_yp",pba->has_yp);
  class_store_columntitle(titles,"(.)rho_yr",pba->has_yr);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_yp],pba->has_yp,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_rho_yr],pba->has_yr,storeidx);
...
int background_derivs(
 ...
 if (pba->has_yp == _TRUE_) {
    /** - compute y particle density \f$ d\rho/dloga = -3 \rho - \Gamma/H \rho \f$*/
    dy[pba->index_bi_rho_yp] = -3.*y[pba->index_bi_rho_yp] - pba->Gamma_yp/H*y[pba->index_bi_rho_yp];
  }

  if ((pba->has_yp == _TRUE_) && (pba->has_yr == _TRUE_)) {
    dy[pba->index_bi_rho_yr] = -4.*y[pba->index_bi_rho_yr]+pba->Gamma_yp/H*y[pba->index_bi_rho_yp];
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->has_yp == _TRUE_) {
      class_print_species("Decaying Y particle",yp);
      budget_matter+=pba->Omega0_yp;
    }
