...
int thermodynamics_helium_from_bbn(
  ...
  double * YHe_at_rhoyh=NULL;
  double * ddYHe_at_rhoyh=NULL;
  double * DH=NULL;
  double * ddDH=NULL;
  double * DH_at_rhoyh=NULL;
  double * ddDH_at_rhoyh=NULL;
  double * DH_at_deltaN=NULL;
  double * ddDH_at_deltaN=NULL;

  int array_line=0;
  double DeltaNeff;
  double omega_b;
  double a_bbn, rhoY_BBN;
  int last_index;
  double Neff_bbn, z_bbn, * pvecback;
  int n;
  double Omega0_b_prime;


  /**--------------------------------------------------------------------------------------------------------**/
  /** - If there is Y decay, infer what rho_Y at the time of BBN via rho_Y at time of decay (reheating) */
  /**--------------------------------------------------------------------------------------------------------**/
  if (pba->has_yp == _TRUE_) {
    class_alloc(pvecback,pba->bg_size*sizeof(double),pba->error_message);
    a_bbn = ((_eV_over_Kelvin_*pba->T_cmb)/(0.1*1.e6))*pow(3.93709/4.7866063,1./3.)*pow(pba->Omega0_g/pba->Omega0_g_prime,-1./4.);
    z_bbn = (1./a_bbn) -1.;
    ...
    Neff_bbn = round( ((pvecback[pba->index_bg_Omega_r]
              *pvecback[pba->index_bg_rho_crit]
              -pvecback[pba->index_bg_rho_g])
    /(7./8.*pow(4./11.,4./3.)*pvecback[pba->index_bg_rho_g]) )*1e5 )/1e5 ;


    rhoY_BBN = 3.*pow(1000.,2.)*( pvecback[pba->index_bg_rho_yp] )/_Mpc_over_GeV_/_Mpc_over_GeV_/8./_PI_/6.7087e-45 ;  // MeV^4 
    //printf("rhoY_BBN [MeV^4] is %.13f\n", rhoY_BBN);
    //printf("a_BBN is %.13f\n", a_bbn);

    free(pvecback);

    /**--------------------------------------------------------------------------------------------------------**/
        // Calculate helium abundance
    /**--------------------------------------------------------------------------------------------------------**/

    /* the following file is assumed to contain (apart from comments and blank lines):
     - the 2 numbers (num_omegab, num_rhoyh) = number of values of BBN free parameters
     - four columns (omegab, rhoyh, YHe) where omegab = Omega0_b h^2 and rhoyh = rho_Y at BBN [MeV^4]
     - omegab and rhoyh are assumed to be arranged as:
     omegab1 rhoyh1 YHe
     omegab2 rhoyh1 YHe
     .....
     omegab1 rhoyh2 YHe
    ...
    class_open(fA,ppr->sBBN_withY_file, "r",pth->error_message);
    //printf("Using BBN file %s\n", ppr->sBBN_withY_file);
    ...
    while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
      ...
      if (left[0] > 39) {
        ...
        /* if the line contains data, we must interpret it. If (num_omegab, num_deltaN, num_rhoyh)=(0,0,0), the current line must contain
          their values. Otherwise, it must contain (omegab, rhoyh, YHe). */
        if ((num_omegab==0) && (num_rhoyh==0)) {

          /* read (num_omegab, num_rhoyh), infer size of arrays and allocate them */
          class_test(sscanf(line,"%d %d",&num_omegab,&num_rhoyh) != 2,
                    pth->error_message,
                    "could not read value of parameters (num_omegab,num_rhoyh) in file %s\n",ppr->sBBN_withY_file);

          class_alloc(omegab,num_omegab*sizeof(double),pth->error_message);
          class_alloc(rhoyh,num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(YHe,num_omegab*num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(ddYHe,num_omegab*num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(YHe_at_rhoyh,num_omegab*sizeof(double),pth->error_message);
          class_alloc(ddYHe_at_rhoyh,num_omegab*sizeof(double),pth->error_message);
          array_line=0;
        }
        else{

          /* read (omegab, rhoyh, YHe) */
          class_test(sscanf(line,"%lg %lg %lg",&(omegab[array_line%num_omegab]),
                                              &(rhoyh[array_line/num_omegab]),
                                              &(YHe[array_line])
                            ) != 3,
                    pth->error_message,
                    "could not read value of parameters (omegab,rhoyh,YHe) in file %s\n",ppr->sBBN_withY_file);
                    //"could not read value of parameters (omegab,rhoyh,YHe) in file %s\n",ppr->sBBN_file);
          array_line ++;
        }
      }
    ...
    /** - spline in one dimension (along rhoyh) */
    class_call(array_spline_table_lines(rhoyh,
                                        num_rhoyh,
                                        YHe,
                                        num_omegab,
                                        ddYHe,
    ...
    class_test(omega_b < omegab[0],
              pth->error_message,
              "You have asked for an unrealistic small value omega_b = %e. The smallest value of omega_b in the lookup table is omegab[0] = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
              omega_b, omegab[0]);

    class_test(omega_b > omegab[num_omegab-1],
              pth->error_message,
              "You have asked for an unrealistic high value omega_b = %e. The largest value of omega_b in the lookup table is omegab[num_omegab-1] = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
              omega_b,omegab[num_omegab-1]);

    class_test(rhoY_BBN < rhoyh[0],
              pth->error_message,
              "You have asked for an unrealistic small value of rho_Y(BBN) = %e. The smallest value of rhoy in the lookup table is rhoyh[0] = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
              rhoY_BBN, rhoyh[0]);

    class_test(rhoY_BBN > rhoyh[num_rhoyh-1],
              pth->error_message,
              "You have asked for an unrealistic high value of rho_Y(BBN) = %e. The largest value of rhoy in the lookup table is rhoyh[num_rhoyh-1] = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
              rhoY_BBN, rhoyh[num_rhoyh-1]);
    ...
    free(ddYHe_at_rhoyh);
      ...
      /**--------------------------------------------------------------------------------------------------------**/
          // Calculate deuterium abundance
      /**--------------------------------------------------------------------------------------------------------**/
    int num_omegab=0;
    int num_rhoyh=0;
    /* the following file is assumed to contain (apart from comments and blank lines):
     - the 2 numbers (num_omegab, num_rhoyh) = number of values of BBN free parameters
     - four columns (omegab, rhoyh, D/H) where omegab = Omega0_b h^2 and rhoyh = rho_Y at BBN [MeV^4]
     - omegab and rhoyh are assumed to be arranged as:
     omegab1 rhoyh1 D/H
     omegab2 rhoyh1 D/H
     .....
    ...
    class_open(fA,ppr->sBBN_withY_deuterium_file, "r",pth->error_message);
    //printf("Using BBN file %s\n", ppr->sBBN_withY_file);
    //class_open(fA,ppr->sBBN_file, "r",pth->error_message);
    ...
    while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
      ...
      if (left[0] > 39) {
        ...
        if ((num_omegab==0) && (num_rhoyh==0)) {
          ...
          class_test(sscanf(line,"%d %d",&num_omegab,&num_rhoyh) != 2,
                    pth->error_message,
                    "could not read value of parameters (num_omegab,num_rhoyh) in file %s\n",ppr->sBBN_withY_deuterium_file);

          class_alloc(omegab,num_omegab*sizeof(double),pth->error_message);
          class_alloc(rhoyh,num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(DH,num_omegab*num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(ddDH,num_omegab*num_rhoyh*sizeof(double),pth->error_message);
          class_alloc(DH_at_rhoyh,num_omegab*sizeof(double),pth->error_message);
          class_alloc(ddDH_at_rhoyh,num_omegab*sizeof(double),pth->error_message);
          array_line=0;
        }
        else{

          /* read (omegab, rhoyh, D/H) */
          class_test(sscanf(line,"%lg %lg %lg",&(omegab[array_line%num_omegab]),
                                              &(rhoyh[array_line/num_omegab]),
                                              &(DH[array_line])
                            ) != 3,
                    pth->error_message,
                    "could not read value of parameters (omegab,rhoyh,D/H) in file %s\n",ppr->sBBN_withY_deuterium_file);
                    //"could not read value of parameters (omegab,rhoyh,YHe) in file %s\n",ppr->sBBN_file);
          array_line ++;
        }
      }
  ...
  /**--------------------------------------------------------------------------------------------------------**/
    ...
    */
    ...
    int num_deltaN=0;
    /* the following file is assumed to contain (apart from comments and blank lines):
     - the 2 numbers (num_omegab, num_deltaN) = number of values of BBN free parameters
     - four columns (omegab, deltaN, D/H) where omegab = Omega0_b h^2 and deltaN = Neff-3.046 by definition
     - omegab and deltaN are assumed to be arranged as:
     omegab1 deltaN1 D/H
     omegab2 deltaN1 D/H
     .....
    ...
    class_open(fA,ppr->sBBN_deuterium_file, "r",pth->error_message);
    //printf("Using BBN file %s\n", ppr->sBBN_withY_file);
    //class_open(fA,ppr->sBBN_file, "r",pth->error_message);
    ...
    while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
      ...
      if (left[0] > 39) {
        ...
        /* if the line contains data, we must interpret it. If (num_omegab, num_deltaN)=(0,0,0), the current line must contain
          their values. Otherwise, it must contain (omegab, delatN, DH). */
        if ((num_omegab==0) && (num_deltaN==0)) {

          /* read (num_omegab, num_deltaN), infer size of arrays and allocate them */
          class_test(sscanf(line,"%d %d",&num_omegab,&num_deltaN) != 2,
                    pth->error_message,
                    "could not read value of parameters (num_omegab,num_deltaN) in file %s\n",ppr->sBBN_deuterium_file);

          class_alloc(omegab,num_omegab*sizeof(double),pth->error_message);
          class_alloc(deltaN,num_deltaN*sizeof(double),pth->error_message);
          class_alloc(DH,num_omegab*num_deltaN*sizeof(double),pth->error_message);
          class_alloc(ddDH,num_omegab*num_deltaN*sizeof(double),pth->error_message);
          class_alloc(DH_at_deltaN,num_omegab*sizeof(double),pth->error_message);
          class_alloc(ddDH_at_deltaN,num_omegab*sizeof(double),pth->error_message);
          array_line=0;
        }
        else{

          /* read (omegab, deltaN, D/H) */
          class_test(sscanf(line,"%lg %lg %lg",&(omegab[array_line%num_omegab]),
                                              &(deltaN[array_line/num_omegab]),
                                              &(DH[array_line])
                            ) != 3,
                    pth->error_message,
                    "could not read value of parameters (omegab,deltaN,D/H) in file %s\n",ppr->sBBN_deuterium_file);
          array_line ++;
        }
      }
    }

    //printf("DeltaNeff is %-15g\n", DeltaNeff);
    //printf("omega_b is %-15g\n", omega_b);
    fclose(fA);
