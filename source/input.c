...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* ** ADDITIONAL SPECIES ** */
   ...
   /** 9) Decaying hidden sector Y particle. Decays into photons and/or DR */
  /* Read */
  class_call(parser_read_double(pfc,"Gamma_yp",&param1,&flag1,errmsg),                          // [1/Mpc]
              errmsg,
              errmsg);
  ...
  class_call(parser_read_double(pfc,"m_ncdm",&param4,&flag4,errmsg),
               ...
               errmsg);
  /* Test */
  class_test(((flag1 == _TRUE_) && (flag2 == _FALSE_)),
              errmsg,
              "In input file, you need both Gamma_yp and R_Gamma_yp, not just one");
  /* Test */
  class_test(((flag1 == _FALSE_) && (flag2 == _TRUE_)),
              errmsg,
              "In input file, you need both Gamma_yp and R_Gamma_yp, not just one");

  /* Complete set of parameters */
  if (flag1 == _TRUE_){
    pba->Gamma_yp = param1;             //[1/Mpc]
  }
  if (flag2 == _TRUE_){
 ...
 // }
  ...
  /*If there is Y decay injecting DR, Omega0_dr needs to be accounted for.*/
  /* This is taken care of by looping over the background calculation in background.c*/
